...
function _M:Connect(handler,luatable)
    ...
    table.insert(self.luatables, luatable)
    local t = { }
    t.Disconnect = function()
        return self:Disconnect(handler, luatable)
    end
...
function _M:Disconnect(handler, luatable)
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
    ...
    if not handler then
        ...
        for k, v in pairs(self.handlers) do
            if v == handler and self.luatables[k] == luatable then
                self.luatables[k] = nil
                self.handlers[k] = nil
                return k
            end
        end
...
assert(self.handlers, "no handler table")
    for k, v in pairs(self.handlers) do
        i = i + 1
        if tools.Debug == false then
            spawn(function() 
                v(self.luatables[k], unpack(self.args)) 
                i = i - 1
