...
function Event.AddListener(event,handler,luatable)
	...
	--conn this handler
	events[event]:connect(handler,luatable)
end
...
function Event.RemoveListener(event,handler,luatable)
	if not events[event] then
		error("remove " .. event .. " has no event.")
	else
		events[event]:disconnect(handler,luatable)
	end
end
