    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        public void CreatePrefabPool(PrefabPool prefabPool)
        {
            ...
            if (!isAlreadyPool)
            {
                ...
                prefabPool.prefab.name = prefabPool.prefab.name.Replace("(Clone)", "");
        ...
        public void RemoveItem(string poolName)
        {
            PrefabPool pool = this.GetPrefabPool(this.prefabs[poolName]);
            if (pool != null)
            {
                for (int i = 0; i < pool._despawned.Count; i++)
                {
                    if (pool._despawned[i] != null)
                    {
                        Transform tran = pool._despawned[i];
                        if(tran.name.Substring(0, tran.name.IndexOf("(")) == poolName)
                        {
                ...
                for (int j = 0; j < pool._spawned.Count; j++)
                {
                    if (pool._spawned[j] != null)
                    {
                        Transform spanwnTran = pool._spawned[j];
                        if (spanwnTran.name.Substring(0, spanwnTran.name.IndexOf("(")) == poolName)
                        {
                            GameObject.DestroyImmediate(spanwnTran.gameObject);
                        }
                    }
                }
                pool._despawned.Clear();
                pool._spawned.Clear();

                this._prefabPools.Remove(pool);
                this.prefabs._Remove(poolName);
            }
        }
        ...
        public void Remove(string name)
        {
            ...
            if (pool != null)
            {
                ...
                for (int i = 0; i < pool._despawned.Count; i++)
                {
                    if (pool._despawned[i] != null && pool._despawned[i].name == name)
                    {
                        index = i;
                        break;
                    }
                }
                if (pool._despawned[index] == null)
                {
                    this._prefabPools.Remove(pool);
                    this.prefabs._Remove(poolName);
                    return;
                }
                GameObject.DestroyImmediate(pool._despawned[index].gameObject);
                pool._despawned.RemoveAt(index);
...
//                                         instance.name));
        ...
        {
            //这里的instance.parent = null;是防止NGUI中的GameObject放到对象池中，修改了PoolManager的父节点
