...
internal static class vp_TargetEventHandler
{
	...
	public static void Register(object target, string eventName, Delegate callback, int dictionary)
	{
		...
		if (target == null) { Debug.LogWarning("Warning: (" + Utility.GetErrorLocation(2) + " -> vp_TargetEvent.Register) Target object is null."); return; }
		if (string.IsNullOrEmpty(eventName)) { Debug.LogWarning("Warning: (" + Utility.GetErrorLocation(2) + " -> vp_TargetEvent.Register) Event name is null or empty."); return; }
		if (callback == null) { Debug.LogWarning("Warning: (" + Utility.GetErrorLocation(2) + " -> vp_TargetEvent.Register) Callback is null."); return; }
		if (callback.Method.Name.StartsWith("<")) { Debug.LogWarning("Warning: (" + Utility.GetErrorLocation(2) + " -> vp_TargetEvent.Register) Target events can only be registered to declared methods."); return; }
	...
	public static Delegate GetCallback(object target, string eventName, bool upwards, int d, vp_TargetEventOptions options)
	{
		...
		{
			Debug.LogError("Error: (" + Utility.GetErrorLocation(2) + ") vp_TargetEvent.Send: Name is null or empty.");
			return null;
		}
#endif
	...
	retry:
		...
		if (!((options & vp_TargetEventOptions.IncludeInactive) == vp_TargetEventOptions.IncludeInactive))
		{
			...
			else
			{
				...
				if (component != null)
				{
					if (!component.enabled || !Utility.IsActive(component.gameObject))
					{
						if (upwards)
							goto recursive;
						return null;
	...
	recursive:
		...
		if ((callback == null) && upwards)
		{
			...
			target = Utility.GetParent(target as Component);
	...
	public static void OnNoReceiver(string eventName, vp_TargetEventOptions options)
	{
		if (!((options & vp_TargetEventOptions.RequireReceiver) == vp_TargetEventOptions.RequireReceiver))
			return;
		Debug.LogError("Error: (" + Utility.GetErrorLocation(2) + ") vp_TargetEvent '" + eventName + "' has no receiver!");
	}
					...
					foreach (string eventName in delegates.Keys)
					{
						...
						if (delegates.TryGetValue(eventName, out d))
						{
							...
							foreach (Delegate dd in d.GetInvocationList())
							{
								...
								{
									p += Utility.GetTypeAlias(i.ParameterType) + " " + i.Name + ", ";
								}
								if (p.Length > 0)
									p = p.Remove(p.LastIndexOf(", "));

								s += Utility.GetTypeAlias(dd.Method.ReturnType) + " ";
