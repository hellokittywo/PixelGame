...
#region ComponentPlacement
		...
		protected static void SetupDetectorInScene<T>() where T: ActDetectorBase
		{
			...
			if (component != null)
			{
				...
				{
					int dialogResult = UnityEditor.EditorUtility.DisplayDialogComplex(detectorName + " already exists!", detectorName + " already exists in scene and placed on object \"" + component.gameObject.name + "\". Do you wish to move it to the new Game Object \"" + CONTAINER_NAME + "\" or delete it from scene at all?", "Move", "Delete", "Cancel");
					switch (dialogResult)
					{
						case 0:
